<div class="main">
    ...
    <table style="width: 100%">
        ...
        <tr>
            {{ Form::open(['method' => 'POST', 'action' => 'Admin\editController@editData_footer_posts', 'files' => 'true']) }}
            <?php foreach($footer_posts as $list): ?> 
                {{ Form::text('footer_post_id', $list->footer_post_id, array('hidden')) }}
                <td><?php echo $list->footer_post_id ?></td>
                <td>{{ Form::file('footer_post_img', array('class' => 'width_box')) }}</td>
                <td>{{ Form::text('footer_post_title', $list->footer_post_title, array('class' => 'width_box')) }}</td>
                <td>{{ Form::text('footer_post_date', $list->footer_post_date, array('class' => 'width_box')) }}</td>
